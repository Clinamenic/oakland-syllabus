...
async function startWatching(
  ...
  const watcher = chokidar.watch(".", {
    ...
    ignoreInitial: true,
    ignored: (watchedPath) => {
      // Prevent chokidar from creating watchers for ignored paths (e.g. .quartz/node_modules)
      const relPath = path.isAbsolute(watchedPath)
        ? path.relative(argv.directory, watchedPath)
        : watchedPath
      const normalizedPath = toPosixPath(relPath)

      // globby's ignore matcher expects strictly relative paths and throws on `..`
      ...
      return buildData.ignored(normalizedPath)
    },
  })
